// merge-kagoshima-municipalities.mjs
// 使い方: node merge-kagoshima-municipalities.mjs
// 機能: 鹿児島県の新規市区町村データを municipalities.json に安全に追加する
//       - 既存 cityId との重複はスキップ
//       - 既存データは一切変更・削除しない
...
const RAW_PATH = resolve(__dirname, 'kagoshima-raw.json');

let newKagoshimaData;
try {
  newKagoshimaData = JSON.parse(readFileSync(RAW_PATH, 'utf-8'));
} catch (e) {
  console.error('❌ kagoshima-raw.json を読み込めません:', e.message);
  process.exit(1);
}
...
function main() {
  ...
  const existingKagoshimaIds = new Set(
    existing
      .filter(item => item.prefId === 'kagoshima')
      .map(item => item.cityId)
  );

  console.log(`\n📋 既存の鹿児島県データ: ${existingKagoshimaIds.size} 件`);
  console.log('  既存 cityId:', [...existingKagoshimaIds].join(', '));

  const toAdd = [];
  const skipped = [];

  for (const item of newKagoshimaData) {
    if (existingKagoshimaIds.has(item.cityId)) {
  ...
  const kagoshimaAfter = verified.filter(x => x.prefId === 'kagoshima');
  console.log('\n🎉 マージ完了！');
  console.log('─'.repeat(40));
  console.log(`📊 鹿児島県 合計: ${kagoshimaAfter.length} 件`);
  console.log(`   補助金あり (true) : ${kagoshimaAfter.filter(x => x.subsidy.hasSubsidy === true).length} 件`);
  console.log(`   補助金なし (false): ${kagoshimaAfter.filter(x => x.subsidy.hasSubsidy === false).length} 件`);
  console.log(`   調査中   (null)  : ${kagoshimaAfter.filter(x => x.subsidy.hasSubsidy === null).length} 件`);
