// merge-kochi-municipalities.mjs
// 使い方: node merge-kochi-municipalities.mjs
// 機能: 高知県の新規市区町村データを municipalities.json に安全に追加する
//       - 既存 cityId との重複はスキップ
//       - 既存データは一切変更・削除しない
...
const RAW_PATH = resolve(__dirname, 'kochi-raw.json');

let newKochiData;
try {
  newKochiData = JSON.parse(readFileSync(RAW_PATH, 'utf-8'));
} catch (e) {
  console.error('❌ kochi-raw.json を読み込めません。先に node build-kochi-raw.mjs を実行してください:', e.message);
  process.exit(1);
}
...
function main() {
  ...
  const existingKochiIds = new Set(
    existing
      .filter(item => item.prefId === 'kochi')
      .map(item => item.cityId)
  );

  console.log(`\n📋 既存の高知県データ: ${existingKochiIds.size} 件`);
  console.log('  既存 cityId:', [...existingKochiIds].join(', '));

  const toAdd = [];
  const skipped = [];

  for (const item of newKochiData) {
    if (existingKochiIds.has(item.cityId)) {
  ...
  const kochiAfter = verified.filter(x => x.prefId === 'kochi');
  console.log('\n🎉 マージ完了！');
  console.log('─'.repeat(40));
  console.log(`📊 高知県 合計: ${kochiAfter.length} 件`);
  console.log(`   補助金あり (true) : ${kochiAfter.filter(x => x.subsidy.hasSubsidy === true).length} 件`);
  console.log(`   補助金なし (false): ${kochiAfter.filter(x => x.subsidy.hasSubsidy === false).length} 件`);
  console.log(`   調査中   (null)  : ${kochiAfter.filter(x => x.subsidy.hasSubsidy === null).length} 件`);
