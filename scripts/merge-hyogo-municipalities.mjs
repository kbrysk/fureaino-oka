/**
 * 兵庫県市区町村データを municipalities.json にマージする。
 * 既存の hyogo cityId はスキップする。
...
const rawPath = path.join(root, "scripts/hyogo-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingHyogoIds = new Set(
  existing.filter((m) => m.prefId === "hyogo").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingHyogoIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Hyogo cityIds: ${[...existingHyogoIds].join(", ")}`);
