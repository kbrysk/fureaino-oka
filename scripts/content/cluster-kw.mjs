...
const NOISE = /活用形|古文|古典|連用形|未然形|已然形|終止形|連体形|仮定形|命令形|助動詞|形容動詞|品詞|用言|文語|口語|分子の形|国語|英語|株式会社|有限会社|合同会社|動詞|形容詞/;
// 「見分け方/見分ける」系は文法・真贋鑑定クエリ＝除去。ただし「悪徳業者の見分け方」等の実需は残す
function isNoise(kw) {
  if (NOISE.test(kw)) return true;
  if (/見分け方|見分ける|見分け 方/.test(kw) && !/業者|悪徳|詐欺|優良|信頼/.test(kw)) return true;
  return false;
}
