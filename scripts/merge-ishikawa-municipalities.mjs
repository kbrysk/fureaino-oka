/**
 * 石川県市区町村データを municipalities.json にマージする。
 * 既存の ishikawa cityId はスキップする。
...
const rawPath = path.join(root, "scripts/ishikawa-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingIshikawaIds = new Set(
  existing.filter((m) => m.prefId === "ishikawa").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingIshikawaIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Ishikawa cityIds: ${[...existingIshikawaIds].join(", ")}`);
