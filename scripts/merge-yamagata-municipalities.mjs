/**
 * 山形県市区町村データを municipalities.json にマージする。
 * 既存の yamagata cityId はスキップする。
...
const rawPath = path.join(root, "scripts/yamagata-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingYamagataIds = new Set(
  existing.filter((m) => m.prefId === "yamagata").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingYamagataIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Yamagata cityIds: ${[...existingYamagataIds].join(", ")}`);
