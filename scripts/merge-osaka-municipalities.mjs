/**
 * 大阪府市区町村データを municipalities.json にマージする。
 * 既存の osaka cityId はスキップ（osaka, sakai, higashiosaka, suita, hirakata, ikeda）。
...
const rawPath = path.join(root, "scripts/osaka-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingOsakaIds = new Set(
  existing.filter((m) => m.prefId === "osaka").map((m) => m.cityId)
);
const skipIds = new Set(["osaka", "sakai", "higashiosaka", "suita", "hirakata", "ikeda"]);
existingOsakaIds.forEach((id) => skipIds.add(id));
...
console.log(`Skipping existing Osaka cityIds: ${[...skipIds].join(", ")}`);
