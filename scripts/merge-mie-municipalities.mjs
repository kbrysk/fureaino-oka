/**
 * 三重県市区町村データを municipalities.json にマージする。
 * 既存の mie cityId はスキップする。
...
const rawPath = path.join(root, "scripts/mie-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingMieIds = new Set(
  existing.filter((m) => m.prefId === "mie").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingMieIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Mie cityIds: ${[...existingMieIds].join(", ")}`);
