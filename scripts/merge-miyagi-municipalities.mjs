/**
 * 宮城県市区町村データを municipalities.json にマージする。
 * 既存の miyagi cityId はスキップする。
...
const rawPath = path.join(root, "scripts/miyagi-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingMiyagiIds = new Set(
  existing.filter((m) => m.prefId === "miyagi").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingMiyagiIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Miyagi cityIds: ${[...existingMiyagiIds].join(", ")}`);
