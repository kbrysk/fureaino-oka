/**
 * 鳥取県市区町村データを municipalities.json にマージする。
 * 既存の tottori cityId はスキップする。
...
const rawPath = path.join(root, "scripts/tottori-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingTottoriIds = new Set(
  existing.filter((m) => m.prefId === "tottori").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingTottoriIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Tottori cityIds: ${[...existingTottoriIds].join(", ")}`);
