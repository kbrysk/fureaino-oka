/**
 * 奈良県市区町村データを municipalities.json にマージする。
 * 既存の nara cityId はスキップする。
...
const rawPath = path.join(root, "scripts/nara-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingNaraIds = new Set(
  existing.filter((m) => m.prefId === "nara").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingNaraIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Nara cityIds: ${[...existingNaraIds].join(", ")}`);
