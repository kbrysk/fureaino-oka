/**
 * 和歌山県市区町村データを municipalities.json にマージする。
 * 既存の wakayama cityId はスキップする。
...
const existingWakayamaIds = new Set(
  existing.filter((m) => m.prefId === "wakayama").map((m) => m.cityId)
);
...
function transform(entry) {
  ...
  const conditionsVal = Array.isArray(s.conditions) && s.conditions.length ? s.conditions : (typeof s.conditions === "string" ? [s.conditions] : undefined);
  const subsidy = {
...
const toAdd = raw.filter((e) => !existingWakayamaIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Wakayama cityIds: ${[...existingWakayamaIds].join(", ")}`);
