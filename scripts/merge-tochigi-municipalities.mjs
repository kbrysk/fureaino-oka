/**
 * 栃木県市区町村データを municipalities.json にマージする。
 * 既存の tochigi cityId はスキップする。
...
const rawPath = path.join(root, "scripts/tochigi-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingTochigiIds = new Set(
  existing.filter((m) => m.prefId === "tochigi").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingTochigiIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Tochigi cityIds: ${[...existingTochigiIds].join(", ")}`);
