/**
 * 静岡県市区町村データを municipalities.json にマージする。
 * 既存の shizuoka cityId（shizuoka, hamamatsu, fujinomiya 等）はスキップする。
...
const rawPath = path.join(root, "scripts/shizuoka-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingShizuokaIds = new Set(
  existing.filter((m) => m.prefId === "shizuoka").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingShizuokaIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Shizuoka cityIds: ${[...existingShizuokaIds].join(", ")}`);
