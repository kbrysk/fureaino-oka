/**
 * 山梨県市区町村データを municipalities.json にマージする。
 * 既存の yamanashi cityId はスキップする。
...
const rawPath = path.join(root, "scripts/yamanashi-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingYamanashiIds = new Set(
  existing.filter((m) => m.prefId === "yamanashi").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingYamanashiIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Yamanashi cityIds: ${[...existingYamanashiIds].join(", ")}`);
