/**
 * 千葉県市区町村データを municipalities.json にマージする。
 * 既存の chiba cityId はスキップする。
...
const rawPath = path.join(root, "scripts/chiba-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingChibaIds = new Set(
  existing.filter((m) => m.prefId === "chiba").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingChibaIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Chiba cityIds: ${[...existingChibaIds].join(", ")}`);
