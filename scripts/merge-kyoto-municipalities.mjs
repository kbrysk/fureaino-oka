/**
 * 京都府市区町村データを municipalities.json にマージする。
 * 既存の kyoto cityId はスキップする。
...
const rawPath = path.join(root, "scripts/kyoto-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingKyotoIds = new Set(
  existing.filter((m) => m.prefId === "kyoto").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingKyotoIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Kyoto cityIds: ${[...existingKyotoIds].join(", ")}`);
