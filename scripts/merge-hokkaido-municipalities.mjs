/**
 * 北海道市区町村データを municipalities.json にマージする。
 * 既存の hokkaido cityId はスキップする。
...
const rawPath = path.join(root, "scripts/hokkaido-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingHokkaidoIds = new Set(
  existing.filter((m) => m.prefId === "hokkaido").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingHokkaidoIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Hokkaido cityIds: ${[...existingHokkaidoIds].join(", ")}`);
