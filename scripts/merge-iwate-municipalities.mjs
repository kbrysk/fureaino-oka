/**
 * 岩手県市区町村データを municipalities.json にマージする。
 * 既存の iwate cityId はスキップする。
...
const rawPath = path.join(root, "scripts/iwate-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingIwateIds = new Set(
  existing.filter((m) => m.prefId === "iwate").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingIwateIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Iwate cityIds: ${[...existingIwateIds].join(", ")}`);
