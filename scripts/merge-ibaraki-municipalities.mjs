/**
 * 茨城県市区町村データを municipalities.json にマージする。
 * 既存の ibaraki cityId はスキップする。
...
const rawPath = path.join(root, "scripts/ibaraki-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingIbarakiIds = new Set(
  existing.filter((m) => m.prefId === "ibaraki").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingIbarakiIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Ibaraki cityIds: ${[...existingIbarakiIds].join(", ")}`);
