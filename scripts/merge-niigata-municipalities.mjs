/**
 * 新潟県市区町村データを municipalities.json にマージする。
 * 既存の niigata cityId はスキップする。
...
const rawPath = path.join(root, "scripts/niigata-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingNiigataIds = new Set(
  existing.filter((m) => m.prefId === "niigata").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingNiigataIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Niigata cityIds: ${[...existingNiigataIds].join(", ")}`);
