/**
 * 神奈川県市区町村データを municipalities.json にマージする。
 * 既存の kanagawa cityId はスキップする。
...
const rawPath = path.join(root, "scripts/kanagawa-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingKanagawaIds = new Set(
  existing.filter((m) => m.prefId === "kanagawa").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingKanagawaIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Kanagawa cityIds: ${[...existingKanagawaIds].join(", ")}`);
