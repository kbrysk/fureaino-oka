/**
 * 群馬県市区町村データを municipalities.json にマージする。
 * 既存の gunma cityId はスキップする。
...
const rawPath = path.join(root, "scripts/gunma-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingGunmaIds = new Set(
  existing.filter((m) => m.prefId === "gunma").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingGunmaIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Gunma cityIds: ${[...existingGunmaIds].join(", ")}`);
