/**
 * 岡山県市区町村データを municipalities.json にマージする。
 * 既存の okayama cityId はスキップする。
...
const rawPath = path.join(root, "scripts/okayama-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingOkayamaIds = new Set(
  existing.filter((m) => m.prefId === "okayama").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingOkayamaIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Okayama cityIds: ${[...existingOkayamaIds].join(", ")}`);
