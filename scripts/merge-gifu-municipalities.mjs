/**
 * 岐阜県市区町村データを municipalities.json にマージする。
 * 既存の gifu cityId はスキップする。
...
const rawPath = path.join(root, "scripts/gifu-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingGifuIds = new Set(
  existing.filter((m) => m.prefId === "gifu").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingGifuIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Gifu cityIds: ${[...existingGifuIds].join(", ")}`);
