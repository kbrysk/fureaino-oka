/**
 * 秋田県市区町村データを municipalities.json にマージする。
 * 既存の akita cityId はスキップする。
...
const rawPath = path.join(root, "scripts/akita-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingAkitaIds = new Set(
  existing.filter((m) => m.prefId === "akita").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingAkitaIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Akita cityIds: ${[...existingAkitaIds].join(", ")}`);
