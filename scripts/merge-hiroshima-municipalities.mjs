/**
 * 広島県市区町村データを municipalities.json にマージする。
 * 既存の hiroshima cityId はスキップする。
...
const rawPath = path.join(root, "scripts/hiroshima-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingHiroshimaIds = new Set(
  existing.filter((m) => m.prefId === "hiroshima").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingHiroshimaIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Hiroshima cityIds: ${[...existingHiroshimaIds].join(", ")}`);
