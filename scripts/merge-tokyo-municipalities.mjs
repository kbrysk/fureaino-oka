/**
 * 東京都市区町村データを municipalities.json にマージする。
 * 既存の tokyo cityId はスキップする。
...
const rawPath = path.join(root, "scripts/tokyo-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingTokyoIds = new Set(
  existing.filter((m) => m.prefId === "tokyo").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingTokyoIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Tokyo cityIds: ${[...existingTokyoIds].join(", ")}`);
