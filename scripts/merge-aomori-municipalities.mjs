/**
 * 青森県市区町村データを municipalities.json にマージする。
 * 既存の aomori cityId はスキップする。
...
const rawPath = path.join(root, "scripts/aomori-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingAomoriIds = new Set(
  existing.filter((m) => m.prefId === "aomori").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingAomoriIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Aomori cityIds: ${[...existingAomoriIds].join(", ")}`);
