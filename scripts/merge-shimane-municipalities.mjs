/**
 * 島根県市区町村データを municipalities.json にマージする。
 * 既存の shimane cityId はスキップする。
...
const rawPath = path.join(root, "scripts/shimane-raw.json");

const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const existingShimaneIds = new Set(
  existing.filter((m) => m.prefId === "shimane").map((m) => m.cityId)
);
...
const toAdd = raw.filter((e) => !existingShimaneIds.has(e.cityId)).map(transform);
console.log(`Skipping existing Shimane cityIds: ${[...existingShimaneIds].join(", ")}`);
